/**
 * Task 12: customer invoice list/detail parity + staff draft editor.
...
assert.match(app, /canViewCompanyInvoices/);
...
assert.match(companyApi, /\/company\/invoices/);
assert.doesNotMatch(companyApi, /\/company\/invoices\/\$\{encodeURIComponent\(invoiceId\)\}\/send/);
...
assert.match(platformApi, /attachBillingAdjustmentIds/);

console.log('Draft invoice frontend checks passed');
