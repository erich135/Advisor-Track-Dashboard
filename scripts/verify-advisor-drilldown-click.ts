...
const topPerformer = await mount(
  '/',
  h(AdvisorNameLink, { advisorId: MAYA_ID, name: 'Maya Brooks', returnPath: '/' })
);
await topPerformer.clickName('Maya Brooks');
assert.equal(topPerformer.location(), `/advisors/${MAYA_ID}?return=${encodeURIComponent('/')}`);
assert.equal(topPerformer.advisorId(), MAYA_ID);
assert.equal(topPerformer.backText(), 'Back to Dashboard');
await topPerformer.clickBack();
assert.equal(topPerformer.location(), '/');
await topPerformer.unmount();
