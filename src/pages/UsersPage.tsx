...
export default function UsersPage({
  ...
  async function saveEditor() {
    ...
    const organisationAdmin =
      isEnterpriseSchemaEnabled() && editor.mode === 'create' && form.organisationAdmin === 'yes';
    const sendInvitation =
      editor.mode === 'create' ? isEnterpriseSchemaEnabled() && form.sendInvitation !== 'no' : undefined;
    setSaving(true);
    try {
      if (editor.mode === 'create') {
        const payload = {
        ...
        if (created.invitationRequested === false) {
                  ...
                  <tr key={member.id}>
                    ...
                    <td>
                      <div className="table-actions">
                        ...
                        {isEnterpriseSchemaEnabled() && actions?.resendInvitation ? (
                          <Button
                            type="button"
                            size="sm"
                            disabled={busyId === member.id}
            ...
            <>
              ...
              <Button type="button" variant="primary" onClick={saveEditor} disabled={saving}>
                {saving
                  ? 'Saving…'
                  : editor?.mode === 'create'
                    ? !isEnterpriseSchemaEnabled() || form.sendInvitation === 'no'
                      ? 'Create user'
                      : 'Invite user'
                    : 'Save'}
              </Button>
        ...
        {editor?.mode === 'view' && editor.member ? (
          ...
          <>
            <UserFormFields
              ...
              enterpriseSchemaEnabled={isEnterpriseSchemaEnabled()}
